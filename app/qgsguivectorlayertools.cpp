...
bool QgsGuiVectorLayerTools::startEditing( QgsVectorLayer* layer ) const
{
  ...
  if ( !layer->isEditable() && !layer->isReadOnly() )
  {
    ...
    {
      UavMain::instance()->messageBar()->pushMessage( tr( "Start editing failed" ),
          tr( "Provider cannot be opened for editing" ),
          QgsMessageBar::INFO, UavMain::instance()->messageTimeout() );
      return false;
    }
  ...
  if ( layer->isModified() )
  {
                                       ...
                                       buttons ) )
    {
      ...
      case QMessageBox::Discard:
        UavMain::instance()->mapCanvas()->freeze( true );
        if ( !layer->rollBack() )
        {
          UavMain::instance()->messageBar()->pushMessage( tr( "Error" ),
...
void QgsGuiVectorLayerTools::commitError( QgsVectorLayer* vlayer ) const
{
  ...
  QgsMessageBarItem *errorMsg = new QgsMessageBarItem(
    ...
    0,
    UavMain::instance()->messageBar() );
	UavMain::instance()->messageBar()->pushItem( errorMsg );
