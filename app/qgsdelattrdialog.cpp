    ...
    for ( int idx = 0; idx < layerAttributes.count(); ++idx )
    {
      ...
      switch ( vl->fields().fieldOrigin( idx ) )
      {
        case QgsFields::OriginExpression:
          item->setIcon( uavCore::getThemeIcon( "mIconExpression.svg" ) );
          break;

        case QgsFields::OriginJoin:
          item->setIcon( uavCore::getThemeIcon( "propertyicons/join.png" ) );
          item->setFlags( item->flags() & ~Qt::ItemIsEnabled );
          break;

        default:
          item->setIcon( uavCore::getThemeIcon( "propertyicons/attributes.png" ) );
