    ...
    , mDock( nullptr )
{
  ...
  mDock = new QDockWidget( tr( "Identify Results" ), UavMain::instance() );
  mDock->setObjectName( "IdentifyResultsDock" );
  mDock->setAllowedAreas( Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea );
  mDock->setWidget( this );
  if ( !UavMain::instance()->restoreDockWidget( mDock ) )
    UavMain::instance()->addDockWidget( Qt::RightDockWidgetArea, mDock );
  else
    UavMain::instance()->panelMenu()->addAction( mDock->toggleViewAction() );
...
void QgsIdentifyResultsDialog::addFeature( QgsVectorLayer *vlayer, const QgsFeature &f, const QMap<QString, QString> &derivedAttributes )
{
  ...
  if ( !vlayer->fields().isEmpty() || vlayer->actions()->size() || !registeredActions.isEmpty() )
  {
    ...
    if ( vlayer->fields().size() > 0 )
    {
      ...
      editItem->setIcon( 0, uavCore::getThemeIcon( "mActionPropertyItem.png" ) );
    ...
    for ( int i = 0; i < vlayer->actions()->size(); i++ )
    {
      ...
      twi->setIcon( 0, uavCore::getThemeIcon( "mAction.svg" ) );
...
void QgsIdentifyResultsDialog::contextMenuEvent( QContextMenuEvent* event )
{
  ...
  if ( featItem )
  {
    if ( vlayer )
    {
      mActionPopup->addAction(
        uavCore::getThemeIcon( "mActionPropertyItem.png" ),
  ...
  if ( featItem && vlayer && vlayer->actions()->size() > 0 )
  {
    ...
    for ( int i = 0; i < vlayer->actions()->size(); i++ )
    {
      ...
      mActionPopup->addAction( uavCore::getThemeIcon( "mAction.svg" ), action.name(), a, SLOT( execute() ) );
    }
  }
  ...
  if ( featItem && vlayer )
  {
    ...
    if ( !registeredActions.isEmpty() )
    {
      ...
      for ( actionIt = registeredActions.begin(); actionIt != registeredActions.end(); ++actionIt )
      {
        ...
        mActionPopup->addAction( uavCore::getThemeIcon( "mAction.svg" ), ( *actionIt )->text(), a, SLOT( execute() ) );
      }
    }
  }
...
{
  connect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), UavMain::instance(), SLOT( setActiveLayer( QgsMapLayer * ) ) );
  emit activateLayer( layer( lstResults->currentItem() ) );
  disconnect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), UavMain::instance(), SLOT( setActiveLayer( QgsMapLayer * ) ) );
}
...
void QgsIdentifyResultsDialog::layerProperties( QTreeWidgetItem *item )
{
  ...
  UavMain::instance()->showLayerProperties( vlayer );
}
